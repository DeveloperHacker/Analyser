...
from tensorflow.python.ops.rnn import _reverse_seq, _rnn_step, _infer_state_dtype, _state_size_with_prefix
...
def bidirectional_rnn(cell_fw, cell_bw, inputs,
    ...
    Returns:
      A tuple (outputs, output_states_fw, output_states_bw) where:
        outputs is a length `T` list of outputs (one for each input), which
          are depth-concatenated forward and backward outputs.
        output_states_fw is the final state of the forward rnn.
        output_states_bw is the final state of the backward rnn.
    ...
    with vs.variable_scope(scope or "BiRNN"):
        ...
        with vs.variable_scope("FW") as fw_scope:
            output_fw, output_states_fw = rnn(cell_fw, inputs, initial_state_fw, dtype,
                                              sequence_length, scope=fw_scope)

        # Backward direction
        with vs.variable_scope("BW") as bw_scope:
            reversed_inputs = _reverse_seq(inputs, sequence_length)
            tmp, output_states_bw = rnn(cell_bw, reversed_inputs, initial_state_bw,
