...
from analyser.attention_dynamic_rnn import attention_dynamic_rnn
from analyser.attention_dynamic_rnn import stack_attention_tree_dynamic_rnn, stack_attention_dynamic_rnn
...
def sequence_input(cell_fw, cell_bw, inputs, inputs_length, hidden_size=None,
    ...
    with vs.variable_scope(scope or "SequenceInput", dtype=dtype) as scope:
        ...
        _, states = bidirectional_dynamic_rnn(cell_fw, cell_bw, hiddens, inputs_length, dtype=dtype)
        states = array_ops.concat(states, 2)
    return states


def labels_output(cell, attention, output_size, output_length, initial_state=None,
                  hidden_size=None, loop_function=None, dtype=None, scope=None):
